...
- 当事者（横塚）が C. 追い上げ型に該当 → 動画末尾で **自己仮説の答え合わせ**
...
- **テンプレ無しの動的生成**（横塚指示「AI に勝手に考えさせる」、固定セリフはハッカソンの作為になる）
...
- 5 つの新規概念（streak_bonus / chain_anxiety / disappointment_stress / miren_uchi / afterglow）→ 横塚指示「特に必要ない」、動画品質確定後に検討
...
4. **spike_v44_demo.py の Phase 構造を 8 段階に再設計** — 横塚壁打ち反映:
   ...
   - Phase 6 (12s): **要因ランキング（束集計版）** — 横塚指摘「件数だけじゃない？」
   - Phase 7 (8s): 仮説支持 4 軸（完全支持）
   - Phase 8 (7s): **仮説への反論 3 軸** — 横塚指示「自己成就回避」
   - 合計 110 秒
...
- 🔴 **LLM 吹き出し統合（Phase 3 演出強化）** — 横塚が「シミュ実行中が暇すぎる」と
...
**横塚原則（次タブで参照）**:
- 動画は「第三者が見て分かる」が最優先
- Claude Code から foreground GUI 起動不可、デスクトップ複製 + 横塚ダブルクリックが確実
- 数字のスケール感は実機経験者の横塚に校正してもらう
