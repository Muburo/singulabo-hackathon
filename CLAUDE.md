...
- v4.1 (2026-04-28): 作者の実機経験で 5 つの歪みを矯正、ChatGPT 再レビュー反映
- **v4.2 (2026-04-29)**: spike 4 本で動画イメージを掴んだ後、作者が **本当の答え合わせしたいこと** を言語化 → **「ストレス × 大逆転認知 → ドーパミン爆発」** の検証を仕様の主軸に据える
...
| 仕様の出典 | [docs/chatgpt-response-v4.md](docs/chatgpt-response-v4.md) + [docs/chatgpt-response-v4-1.md](docs/chatgpt-response-v4-1.md) + 作者の実機経験（v4.2 で言語化） |

## 動機と仮説

### 動機
作者の長年の実機経験から得た主観的な印象を、抽象モデル上で「答え合わせ」する。
...
## v4.4 仕様（観察型 + 当事者仮説の答え合わせ、2026-05-01〜）

> v4.4 は v4.2/v4.3 の上に **観察型動画 + 8 属性 + 当事者仮説の答え合わせ** を重ねる差分仕様。
> 既存 L1-L1119 の v4.2 仕様は土台として保持、本セクションが v4.4 時点の SSoT。
> 詳細レビューは [docs/chatgpt-response-v4-3.md](docs/chatgpt-response-v4-3.md) 参照。

### v4.4 の核心転換

- 「仮説検証 → 失敗を救う」構造から、**「観察型の群比較 + 当事者仮説の答え合わせ」** に再設計
- 1000 人 × 8 属性均等配分（125 × 8）、リアルなホール分布は再現しない
- 画面表示は **「脳汁」「ストレス」のみ**（arousal / despair / dopamine は内部変数）
- 作者の主観的経験は C. 追い上げ型に近い → 動画末尾で **自己仮説の答え合わせ**
- **「検証」ではなく「抽象モデル上の観察と答え合わせ」** と表現（自己成就回避）

### 8 属性パターン

| # | 表示名 | 軽い説明 | sens | base_stress | personal_threshold | stress_floor | hammari_tau | 追加軸 |
|---|------|---------|------|---------|------|------|------|------|
| A | 新人 | はじめて来店、まだ波が読めない | 0.84 | 0.20 | 30,000 | 0.03 | 18 | novelty_boost |
| B | 常連 | 月数回、機種は決まってる | 0.52 | 0.50 | 60,000 | 0.18 | 14 | habituation 0.80 |
| **C** | **追い上げ** ★ | 取り戻したい、兆しに過敏 | 0.92 | 0.78 | 15,000 | 0.48 | 8 | recovery_relief 1.35, dopamine_boost 1.10 |
| D | アフターファイブ | 仕事帰りの 2 時間 | 0.72 | 0.45 | 50,000 | 0.12 | 12 | time_pressure |
| E | 悠々自適 | 経済的余裕、暇つぶし | 0.35 | 0.08 | 220,000 | 0.00 | 24 | emotion_damp 0.85, ev_focus 0.10 |
| F | シニア常連 | 退職後の長居 | 0.42 | 0.34 | 80,000 | 0.10 | 20 | long_stay |
| G | 息抜き | 家事や日常から離れたい | 0.68 | 0.48 | 35,000 | 0.12 | 12 | relief_weight |
| H | パチプロ | 期待値で打つ、これで生活 | 0.25 | 0.22 | 120,000 | 0.08 | 18 | emotion_damp 0.55, ev_focus 0.95, bad_ev_stress 0.25 |

★ = 当事者仮説の主役。コード内部識別子は `C_CHASE`、Phase 6 の自己仮説テキストでだけ「中毒型に近いと仮定」と表現。

属性内 125 人の散らし: 潜在変数 z（追い詰められやすさ）で sens/stress/threshold を相関させてサンプリング。詳細は [docs/chatgpt-response-v4-3.md](docs/chatgpt-response-v4-3.md) §4「属性内 125 人の散らし方」参照。

### ストレスの動的化（v4.2 静的合成からの差分）

2 層構成: `stress_load = base_stress + situational`、爆発項には必ず `pre_event_stress` を使う。

```text
stress_target = base_stress
              + 0.42 × tanh(loss_over_safe_budget / personal_threshold)   # 損失飽和
              + 0.16 × (1 − exp(−miss_streak / hammari_tau))              # ハマり指数飽和
              + 0.18 × sigmoid((cash_floor − cash) / cash_low_scale)      # 残金圧
              + 0.10 × time_pressure_signal                               # 時間圧（D/G）
              + 0.08 × chain_anxiety                                       # 連チャン中の終わる/取りきれない
              − 0.22 × tanh(recovery_from_bottom / personal_threshold)    # 底からの回復
              − event_relief                                                # chain_start −0.08 / heavy −0.06

stress_next  = 0.72 × pre_event_stress + 0.28 × clip(stress_target)
stress_next  = max(stress_floor, stress_next)
```

完全擬似コードは [docs/chatgpt-response-v4-3.md](docs/chatgpt-response-v4-3.md) §2「最終擬似コード」参照。

### 幸福度・最不幸の定義

```text
joy_score = 0.65 × Δ脳汁 + 0.25 × 脳汁_after + 0.90 × ストレス解放
          + min(1, event_impact) × 8
          − 0.10 × pre_stress × 100

misery_score = 0.65 × stress_after + 0.35 × stress_rise
             + 0.20 × (100 − 脳汁_after) + 0.20 × loss_pressure × 100
             + 0.10 × hammari_pressure × 100 + 8 × cash_low_signal
```

抽出基準を Phase で分ける（**ここを混ぜると視聴者が「何のランキング？」になる**）:

- **Phase 4 最高幸福ランキング** = `max(joy_score)` を属性内で 1 件
- **Phase 5 最不幸ランキング** = `max(misery_score)` を属性内で 1 件
- **Phase 6 仮説答え合わせ** = `max(Δ脳汁)` と `pre_event_stress` の関係（仮説に対してフェアに）

### 「爆発の兆し」の定義（C 仮説の主役概念）

```python
HEAVY_CUE_EVENTS = {"kakutei_engi", "tokka_heavy", "uwanose_heavy"}

def is_explosion_cue(row):
    if row.trigger_event in HEAVY_CUE_EVENTS:
        return True
    if row.trigger_event == "chain_start" and row.machine_name in {"GOD_OKI", "ART_2010"}:
        return row.event_impact >= 0.52
    return row.event_impact >= 0.90
```

呼び分け: `chain_start` = 「爆発の入口」、HEAVY_CUE_EVENTS = 「爆発の兆し」。

### 動画構成 Phase 1-6（合計 110 秒、1 分 50 秒）

| Phase | 内容 | 秒数 |
|-------|------|------|
| 1 | 環境説明 + **私の仮説提示**（4 機種紹介、追い上げ型仮説） | 15 |
| 2 | 8 属性パターン紹介（軽い説明 + 1000 人内訳 125×8） | 15 |
| 3 | シミュ実行（ホール俯瞰、1000 人早回し、抽象アイコン） | 15 |
| 4 | **最高幸福ランキング**（8 属性 × ベスト、各カード 2 秒 + Top1-2 拡大） | 25 |
| 5 | **最不幸ランキング**（8 属性 × ワースト、同構成） | 25 |
| 6 | **仮説答え合わせ**（C 125 人抽出、散布図 + Top5、3 段階合否） | 15 |

各カードの情報は 3 つだけ: 属性名 / 起きたこと / 数値（脳汁・ストレス）。心の声は LLM voice で 1 行（20-34 字）。

### 仮説答え合わせ（Phase 6、Lv2 + 4 つの追加指標）

C 追い上げ 125 人を抽出して以下を測定:

1. Top5 直前ストレス平均 vs C 全体平均 vs 兆しイベント平均
2. 高ストレス × 兆し の平均 Δ脳汁 vs 低ストレス × 兆し
3. 脳汁ピーク（上位 10%）の条件付き確率
4. Spearman 相関（飽和あるので Pearson NG）

**合否判定 3 段階**:

- **支持**: top5_avg ≥ all_c_avg + 0.12 かつ mean_delta_high_cue ≥ mean_delta_low_cue × 1.25
- **部分支持**: top5_avg ≥ all_c_avg + 0.06 または mean_delta_high_cue ≥ mean_delta_low_cue × 1.10
- **今回は支持されず**: それ以外

「失敗」と言わない。テンプレ文言（パターン 1-3）は [docs/chatgpt-response-v4-3.md](docs/chatgpt-response-v4-3.md) §6 参照。

### ablation（自己成就回避）

stress 係数 OFF モデル 1 本を別途回す:

- `dopamine_burst` の `(0.5 + 1.5 × stress)` を `1.0` に固定
- C 型 Top5 が通常モデルとどう変わるかを比較
- PDF と動画末尾で 5 秒程度言及
- 「ストレス項を抜いてもピークは出るが、Top 5 の直前ストレス平均は X → Y に下がった」のような表現

実装は spike_v44_ablation.py で別ファイル化、通常 run と並走。

### LLM voice（few-shot 改良）

ハイライト 16 シーン × 心の声 1 行（fallback 必須）。詳細 prompt は [docs/chatgpt-response-v4-3.md](docs/chatgpt-response-v4-3.md) §5 参照。

**禁止語**: 「また」「やばい」「最高」「終わった」連発、医学・借金・依存・病名・家庭事情、分析口調、属性名そのまま、recent_voices と似た言い回し。

### 提出物の表現指針

- 動画タイトル「**C. 追い上げ型**」、Phase 6 自己仮説でだけ「中毒型に近いと仮定」
- 抽象アイコン（人物アイコン NG）、全属性同じ明度、C を危険色にしない
- **PDF に必須記述**:
  - 「抽象モデルであり、実データの因果検証ではない」
  - 「1000 人は実分布ではなく、8 属性を同じ人数で比較する実験設計」
  - 「配布コードの 2D 空間エージェントシミュレーションを、パチスロホール内の台選択・滞在・離脱に拡張」
- **動画締めの一言**:
  > ストレスは、脳汁を生む原因そのものではなかった。
  > でも、重い兆しが来た時に、それを何倍にも増幅する燃料だった。

### v4.2/v4.3 仕様との関係（差分概要）

| 項目 | v4.2/v4.3 | v4.4 |
|------|------|------|
| stress_load | session 開始で固定 | 動的更新（tanh 飽和 + 慣性 0.72:0.28） |
| 機種 | 4 機種 + 中間イベント | 同（v4.3 schema 維持） |
| 感情 3 軸 | arousal / despair / dissociation | 内部維持、画面表示は脳汁 / ストレスのみ |
| persona | 8 体手書き | 8 属性 × 125 人サンプリング = 1000 人 |
| 仮説 | ストレス × 大逆転認知 | 同（C 型のみ自己仮説として） |
...
作者提案: 「シミュレーションにならないから機種固定」「8 属性 × 6 機種 = 48 セルで観察」
...
## v4.6 動画化仕様（作者壁打ち、2026-05-02）

> v4.6 は v4.5 物理エンジンを土台に、**動画提出物として第三者に伝わる構造**へ再設計した差分仕様。
> 物理パラメータ・48 セル格子・5 つの新規概念は v4.5 を継承。
> 本セクションは表示名・凡例・吹き出し・Phase 構成・金額表示の差分。

### v4.6 で直す動画の致命欠点（作者指摘、2026-05-02）

1. 金額がデタラメ（最終所持金 +20-30 万円勝ちが多発）
2. 人の気持ちが見えない（数字しか出ない）
3. 何が起きてるか分からない（ナレーション欠如）
4. 用語の定義（属性・機種名）が分からない
5. 仮説答え合わせグラフが読み取れない

### 表示名の日本語化（v4.6 核心）

英語コードは **内部 ID のみ**、動画には一切出さない。動画は日本語表示名で統一。

#### 8 属性（動画用、v4.6 確定）

| 内部 ID | 表示名 | 一行説明 |
|--------|-------|---------|
| `A_NEWBIE` | **初心者** | パチンコ屋やパチスロに慣れていない人 |
| `B_REGULAR` | **ライトユーザー** | 月に数回程度行く人たち |
| `C_CHASE` | **中毒者** | パチスロジャンキー。借金もあり、生活に支障をきたすような打ち方 |
| `D_AFTER5` | **アフター5層** | 仕事帰りのサラリーマンなど |
| `E_LEISURE` | **悠々自適** | 経済的に余裕、暇つぶしで打つ地主や不動産持ち |
| `F_SENIOR` | **シニア** | 退職後、時間的な余裕が相当ある人たち |
| `G_BREATHER` | **主婦** | 家事や日常から離れて来てる |
| `H_PRO` | **パチプロ** | 期待値で打つ、これで生活 |

仮説対象: **中毒者**（旧 C 追い上げ型をリネーム、依存・追い上げのネガ表現を回避）

#### 6 機種カテゴリ（動画用、v4.6 確定）

v4.5 コードを以下にリネーム（**物理パラメータは v4.5 据え置き**、コード名・display_name・representatives のみ書き換え）:

| v4.5 旧 ID | v4.6 内部 ID | 表示名 | 説明 | 代表機 |
|-----------|--------------|-------|-----|--------|
| `PURE_A` | `NORMAL_BONUS` | **ノーマル・ボーナス型** | 基本はボーナスのみで抽選 | ジャグラー / ハナハナ / ニューパルサー / クランキーコンドル |
| `BAKURETSU_AT` | `BURST_AT_4G` | **4号機・爆裂AT型** | 初当たり遠いが、入れば爆裂 | 獣王 / サラリーマン金太郎 / アラジンA / 猛獣王 |
| `MIDDLE_45` | `LATE_4G_MASS` | **4号機後期・大量獲得型** | 大量獲得を狙える、中毒性高い | 北斗の拳 / 吉宗 / 押忍！番長 / 鬼浜爆走愚連隊 |
| `ART_2010` | `SELF_TRIGGER_5G` | **5号機・自力契機型** | 規制下でも一撃性を残した自己責任機 | 押忍！サラリーマン番長 / モンキーターン / 修羅の刻 / 戦国乙女 |
| `CHAIN_OKI` | `LOOP_CHAIN_5G` | **5号機・連チャン高継続型** | 引けば続く、終わるときはすぐ終わる | 沖ドキ！ / リノ / 南国育ち |
| `GOD_2000` | `GOD_ORIGIN` | **初代GOD・別格型** | 一撃5000枚、多くの人を不幸にした伝説の機種 | ミリオンゴッド 初代 / ゴールドXR |

代表機は動画内で **2-3 機まで省略表示**（全部羅列するとフォントが小さくなりすぎる）。

### 仮説の言い直し（v4.6）

> **「最もストレスがかかるのは中毒者。ストレスが高い時に爆発の兆しがあった瞬間、最も脳汁が出る」**

### 金額・収支表示

50 step は **半日 4,000G 相当**（1 日 8,000G）として動画内で表記。物理エンジンの stake/payout は v4.5 据え置き、表示時の意味付けのみ変更。

個別カード表示ルール:
- 「**収支 +N 万円 / -N 万円**」のみ（万円単位四捨五入）
- 「最終所持金」「払出枚数」「払出 35,401 枚」表示は **廃止**
- payout 数字は「勝ち額」「負け額」の文脈表現に統一

### 心の声統合（LLM voice、v4.6 で初投入）

Phase 3 シミュ実行中、各 step で **0-2 個ランダム**の吹き出しが各台から出る。

- 使用モデル: `qwen3:4b-instruct-2507-q4_K_M`（v43 で動作確認済、600 call ≈ 3 分）
- プロンプトに **属性 + 機種 + イベント + ストレス + 収支** を入れて生成
- **テンプレ無しの動的生成**（作者指示「AI に勝手に考えさせる」、固定セリフはハッカソンの作為になる）
- 想定セリフ数: 30-40 個 / 50 step

### 凡例領域（右 1/4、Phase 2-6 常時表示）

```
【属性】              【機種カテゴリ】
🟢 初心者             ノーマル・ボーナス型
🔵 ライトユーザー     4号機・爆裂AT型
🔴 中毒者 ← 仮説対象  4号機後期・大量獲得型
🟠 アフター5層        5号機・自力契機型
🩵 悠々自適           5号機・連チャン高継続型
🟣 シニア             初代GOD・別格型
💗 主婦
...
Phase 1（仮説提示）のみ凡例なし。Phase 2 で凡例の意味を説明、以降常時表示。
...
|-------|----|------|
| 1 | 10 | タイトル + 仮説提示（凡例なし） |
| 2 | 12 | 48 セル格子 + **凡例領域の説明**（属性 + 機種カテゴリの常時表示開始） |
| 3 | 25 | シミュ実行 4,000G 相当（吹き出し、左下に直近イベント、右上に凡例） |
| 4 | 20 | 最高幸福ランキング Top 3（収支のみ表示、心の声込み） |
| 5 | 20 | 最高ストレスランキング Top 3 |
| 6 | 18 | 仮説答え合わせ（グラフ副題 + 結論大字幕） |

合計 105 秒。Phase 1.5（独立キャラ紹介）は廃止、Phase 2 + 凡例常時表示で代替。

### 仮説答え合わせ（Phase 6）強化

- グラフ副題を必須化:
  - 「① 属性別 max ストレス → 仮説の前提『中毒者が最ストレスか』を検証」
  - 「② 中毒者 × 機種別 Δ脳汁 → 仮説の核『中毒者は GOD で爆発するか』を検証」
- 画面下に **大字幕**: 「✅ 仮説支持された」 or 「⚠️ 部分支持（中毒者は最ストレスだが、最興奮機種は ◯◯）」

### v4.6 で含めない（後回し）

- 5 つの新規概念（streak_bonus / chain_anxiety / disappointment_stress / miren_uchi / afterglow）→ 作者指示「特に必要ない」、動画品質確定後に検討
- 音声ナレーション（字幕オンリーで提出仕様内）

### v4.5 / v4.6 関係表

| 項目 | v4.5 | v4.6 |
|------|------|------|
| 物理パラメータ | 6 機種完全校正 | **同（不変）** |
| 48 セル格子 | 8 属性 × 6 機種 | **同（不変）** |
| 機種コード | PURE_A 等 英語 | **NORMAL_BONUS 等にリネーム + 日本語表示名** |
| 動画上のコード表示 | 英語混在 | **日本語のみ** |
| 仮説対象 | C 追い上げ型 | **中毒者**（リネーム） |
| 金額表示 | 最終所持金 + 払出 | **収支のみ（万円単位）** |
| 心の声 | 未統合 | **LLM 動的生成（qwen3:4b）** |
| 動画長 | 115 秒 | **105 秒** |
| 凡例 | なし | **右 1/4 常時表示** |
| 5 つの新規概念 | 未統合（5/3 PM 予定） | **未統合（差し替え延期）** |

### v4.6 実装スコープ（次セッション）

1. `spike_v44.py` の MachineType を v4.6 ID へリネーム（NORMAL_BONUS 等）+ display_name + representatives 更新
2. `spike_v44_personas.py` の AttributePattern.display_name を v4.6 表示名へ（新人 → 初心者、追い上げ → 中毒者 等）
3. `spike_v44_demo.py` を v4.6 仕様で全面書き換え:
   - 凡例領域を右 1/4 に常時表示
   - Phase 構成を 105 秒版に再構成
   - 個別カードを「収支のみ」表示に
   - LLM voice 統合（spike_llm_voice.py を呼ぶ）
   - グラフに副題追加、結論大字幕

## 進捗ログ

### 2026-05-02 PM — v4.6 動画化（Phase 1-8 構造確立、自己成就回避まで到達）

**やったこと**:

1. **物理エンジン v4.6 リネーム** — spike_v44.py の MachineType を新 ID へ:
   - PURE_A → NORMAL_BONUS（ノーマル・ボーナス型）
   - MIDDLE_45 → LATE_4G_MASS（4号機後期・大量獲得型）
   - BAKURETSU_AT → BURST_AT_4G（4号機・爆裂AT型）
   - ART_2010 → SELF_TRIGGER_5G（5号機・自力契機型）
   - CHAIN_OKI → LOOP_CHAIN_5G（5号機・連チャン高継続型）
   - GOD_2000 → GOD_ORIGIN（初代GOD・別格型）
   - 物理パラメータは v4.5 据え置き、display_name と representatives を更新

2. **属性表示名の v4.6 化** — spike_v44_personas.py:
   - 新人 → 初心者、常連 → ライトユーザー、追い上げ → **中毒者**
   - シニア常連 → シニア、息抜き → 主婦
   - 「中毒者」= パチスロジャンキー、借金あり、生活に支障をきたすような打ち方

3. **収支スケール調整 v4.6.1** — 作者指摘「半日 +20-39 万勝ちはデタラメ」:
   - stake 1000 → 400、payout_mean/std/cap を全機種 ×0.4
   - target_return 不変、収支絶対値が 0.4 倍に
   - 1 日 8000G 相当として動画表示
   - DURATION_MODE 切替変数追加（half_day / full_day）

4. **spike_v44_demo.py の Phase 構造を 8 段階に再設計** — 作者壁打ち反映:
   - Phase 1 (13s): 脳汁の定義 + 仮説提示
   - Phase 2 (11s): 48 セル格子（一括表示、機種上の 1 行説明）
   - Phase 3 (21s): シミュ実行（LLM 吹き出しは未統合）
   - Phase 4 (13s): 属性別 脳汁ランキング（中毒者 1 位、平均 +31）
   - Phase 5 (25s): 個人 Top 3 詳細（借金ストーリー込み、3位→2位→1位）
   - Phase 6 (12s): **要因ランキング（束集計版）** — 作者指摘「件数だけじゃない？」
   - Phase 7 (8s): 仮説支持 4 軸（完全支持）
   - Phase 8 (7s): **仮説への反論 3 軸** — 作者指示「自己成就回避」
   - 合計 110 秒

5. **要因ランキング束集計（v4.6.4）** — 「契機」が引いた連チャン全体の累積脳汁を
   契機 1 回あたりで平均する設計。瞬間 Δ脳汁ではなく、突入後の連チャン分も含む。
   結果: 重い特化ゾーン突入 +59（5位、瞬間） → +123（2位、束集計、ゴッド除く 1 位）

6. **仮説への反論 3 軸（v4.6.5）** — 自己成就回避:
   - ① Top 5 中の非中毒者件数 → 主婦が Top 2（+217）に入った [反論あり]
   - ② 同機種(GOD) 属性別 max 差 → 1.54 倍、属性差も大きい [支持]
   - ③ 低ストレス（< 0.3）での +50 以上脳汁 → 171 件 [反論あり]
   - 真の知見: 「仮説は支持されたが、機種の力も独立に効いていた」

**学び**:
- 「第三者が見て分かる」が動画提出物の至上命題（日本語化、機種説明、脳汁定義、ストーリー）
- 収支表示は `final_cash - initial_cash` の万円四捨五入「+N 万円」に統一
- 要因ランキングの束集計が肝（瞬間 Δ脳汁だと特化ゾーン突入が過小評価される）
- 仮説検証は支持判定だけでなく反論軸を入れて自己成就を回避

**残課題（次セッション開始点）**:

- 🔴 **LLM 吹き出し統合（Phase 3 演出強化）** — 作者が「シミュ実行中が暇すぎる」と
  指摘した本丸。spike_llm_voice.py（qwen3:4b）を Phase 3 の各 step に統合し、
  AI が状況（属性 + 機種 + イベント + ストレス + 収支）からセリフを動的生成。
  ニコニコ動画式の流れる吹き出し or ポップアップ表示。30-40 個 / 50 step 想定。
- 🟡 Phase 4 の「↑ 仮説対象 中毒者」が 2 位の上に乗ってしまう、レイアウト要修正
- 🟡 Phase 6 の 1 位「+220」が画面端で見切れ、棒グラフ最大幅を 6.0 → 5.5 に縮小
- 🟢 Phase 2 の機種 1 行説明と機種名の間隔（読めるが詰まり気味）
- 🟢 解説 PDF（提出物の 3 点目）未着手 — 5/6 で OK

**作者原則（次タブで参照）**:
- 動画は「第三者が見て分かる」が最優先
- Claude Code から foreground GUI 起動不可、デスクトップ複製 + 作者ダブルクリックが確実
- 数字のスケール感は実機経験者の作者に校正してもらう
- 仮説の自己成就を避ける反論軸を入れる科学的態度

**動画**: `outputs/spike_v44_demo.mp4`（110 秒）+ `~/Desktop/` 複製。提出締切 5/7 17:00。次タブは「v4.6 続き、LLM 吹き出し」で再開。
...
2. **ChatGPT Pro 相談プロンプト v4.4 作成** — `docs/相談プロンプト-v4-4.md`
   - 作者の実機証言（6 機種別 + ストレス 3 パターン）を生のまま添付
   - 5 つの新規概念の式を依頼
3. **ChatGPT Pro v4.5 レビュー受領** — `docs/chatgpt-response-v4-4.md`
   ...
   - 罠 10 個 + 締めの言葉
4. **作者の機種選択ロジック判断**: 8 属性 × 6 機種 = 48 セル格子で観察、各属性 125 人 ÷ 6 機種 = 21+21+21+21+21+20
...
- 機種選択を均等分配することで「属性のせいか機種のせいか」が分離できる（作者の鋭い設計判断）
...
1. **GitHub push 完了** — `Muburo/singulabo-hackathon` (private) 作成、main + experiment ブランチ push
2. **作者の方向転換**:
   - 個別 persona → 8 属性パターン × 1000 人均等配分
   - 観察型 + 当事者仮説の答え合わせ構造
   - 表現の丸さ（嫌悪感回避）
3. **8 属性確定**: A 新人 / B 常連 / **C 追い上げ** / D アフターファイブ / E 悠々自適 / F シニア常連 / G 息抜き / H パチプロ
4. **ChatGPT Pro v4.4 レビュー受領** — `docs/chatgpt-response-v4-3.md`
   - v4.4 方向転換は採用、自己成就回避の警告
   - stress 動的化式（tanh 飽和 + 慣性 0.72:0.28 + chain_anxiety + recovery_signal）の擬似コード降下
   - 幸福度 = `0.65 × Δ脳汁 + 0.25 × 脳汁 + 0.90 × ストレス解放 + イベボーナス − 0.10 × pre_stress`
   - 「中毒」→ 動画名は「追い上げ」、Phase 6 自己仮説でだけ「中毒型に近いと仮定」
   - 「爆発の兆し」= `HEAVY_CUE_EVENTS`（α 主定義）+ 重い機種の chain_start（条件付き）
   - 合否判定 3 段階（支持 / 部分支持 / 今回は支持されず）+ 外れた場合のテンプレ文言 3 パターン
   - ablation（stress 係数 OFF）1 本走らせて Top5 比較
5. **CLAUDE.md に v4.4 仕様セクション追記**（差分追加方式、v4.2 本文 L1-L1119 は無傷）

**学び**:

- 仮説を式に組み込んでから検証すると **自己成就**に見える → Phase 6 は「実データ検証」ではなく「抽象モデル上の答え合わせ」と表現する
- 表現の丸さは **動画タイトルだけ丸めて、内部パラメータは強いまま** が正解（C_CHASE / 追い上げ）
- 「失敗」と言わず「今回は支持されず」と言う方が、観察された傾向の救済テンプレが効きやすい
...
- spike_v44.py 着手（stress 動的化 + 8 属性 + 1000 人サンプリング）
- 残工数 6 日 × Day-by-Day:
  - **5/2**: spike_v44.py 着手（stress 動的化 + 8 属性 + 1000 人）
  - **5/3**: Phase 4-5 動画化（最高幸福・最不幸ランキング）
  - **5/4**: Phase 6 答え合わせ + ablation
  - **5/5**: LLM voice 統合 + few-shot 改良
  - **5/6**: 動画組み立て + PDF 解説（「配布コード拡張」位置づけ含む）
  - **5/7**: 提出（17:00 締切）
...
3. **Codex レビュー → 5 件指摘を全部修正**
   ...
   - **PURE_A から kakutei_engi 完全削除**（作者判断: 純Aプレミア告知は興奮の爆発を伴わない）
...
8. **ハッカソン要項を認識** — 作者から再共有（3-4 回目反省）
   - 提出物: **ソースコード（GitHub 理想）+ 可視化動画 + 簡単な解説文章**
   - 動画長: **1〜2 分**（作者判断、最長 2 分）
...
作者に **A / B / C のどれで Step を進めるか** 確認する。動画 MVP は立ったから、**残り工数 5〜6 日で 1〜2 分の動画提出版を作る**のがゴール。
...
**学び**:
- 「動かす」と「答え合わせの言語化」が連動する。spike を見せた結果、作者が**本当の動機**を言語化できた
...
- **観客視点（作者の感覚）を持って画面を組まないと、専門家向けデータ可視化に陥る**
...
- v4 仕様（ChatGPT Pro 確定）を Phase 1 着手前にレビュー、作者の実機経験で 5 つの歪みを発見
...
作者の方針: **「外側 → 内側」**。画面の見せ方を固めてから、前提条件（仮説・パラメータ・感情モデル）の細部を詰める。
