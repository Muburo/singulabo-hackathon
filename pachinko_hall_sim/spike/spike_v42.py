"""
spike_v42.py — v4.2 仕様の動く実装

「ストレス × 大逆転認知 → ドーパミン爆発」を検証する spike。

実装した v4.2 要素:
- 中間イベント（上乗せ・特化ゾーン・確定演出）
- session context の stress_load（borrow_burden / work_stress / life_dissatisfaction）
- 感情モデルの upset_recognition × stress_load 爆発項
- arousal_delta の記録 + Top 5 ピーク抽出
- stress_load × Δarousal 散布図

表示フロー（3 phase）:
  Phase 1: ライブシミュレーション完走（ホール俯瞰）
  Phase 2: Top 5 ハイライト 1 つずつ表示（各 5 秒）
  Phase 3: stress_load × Δarousal 散布図（仮説検証）
"""
import random
...
class MachineType:
    ...
    payout_cap: float
    # v4.2 中間イベント倍率
    uwanose_mult: float = 0.0
    tokka_mult: float = 0.0
    kakutei_mult: float = 0.0
    stake: int = 1000
...
MACHINE_TYPES = [
    MachineType("GOD",   "#d9534f", "chain", 0.025, 0.50, 21000, 25000, 120000,
                uwanose_mult=1.5, tokka_mult=0.5, kakutei_mult=1.5),
    MachineType("北斗",  "#f0ad4e", "chain", 0.060, 0.58,  8300,  5600,  80000,
                uwanose_mult=1.0, tokka_mult=1.0, kakutei_mult=1.0),
    MachineType("ART",   "#5bc0de", "chain", 0.030, 0.83,  6700,  4000,  60000,
                uwanose_mult=1.2, tokka_mult=2.0, kakutei_mult=0.5),
    MachineType("沖スロ", "#5cb85c", "none",  0.200, 0.00,  5000,  2000,  20000,
                uwanose_mult=0.0, tokka_mult=0.0, kakutei_mult=0.0),
]
    ...
    if mt.chain_mode == "none":
        ...
        if persona.chain_active:
            ...
            if rng.random() < cont_prob:
                ...
                roll = rng.random()
                # 確定演出（最低確率、超強い）
                if roll < 0.01 * mt.kakutei_mult:
                    kakutei_engi = True
                    trigger_event = "kakutei_engi"
                    persona.tokka_zone_active = True
                    persona.tokka_zone_remaining = int(rng.integers(20, 40))
                    uwanose_amount = int(rng.choice([200, 300, 500]))
                # 特化ゾーン
                elif roll < 0.01 * mt.kakutei_mult + 0.04 * mt.tokka_mult:
                    tokka_zone_entry = True
                    trigger_event = "tokka_zone_entry"
                    persona.tokka_zone_active = True
                    persona.tokka_zone_remaining = int(rng.integers(10, 30))
                # 上乗せ
                elif roll < 0.01 * mt.kakutei_mult + 0.04 * mt.tokka_mult + 0.15 * mt.uwanose_mult:
                    uwanose_amount = int(rng.choice([50, 100, 150, 200]))
                    trigger_event = f"uwanose_{uwanose_amount}g"
            else:
                persona.chain_active = False
                persona.chain_step_count = 0
                persona.tokka_zone_active = False
...
def compute_upset_recognition(event_info, persona):
    """v4.2: 大逆転認知シグナル（0.0-1.0）"""
    chain_continued_long = 1.0 if persona.chain_step_count >= 5 else 0.0
    return min(1.0, max(0.0,
        0.40 * (1.0 if event_info["chain_just_started"] else 0.0)
      + 0.60 * (1.0 if "uwanose" in event_info["trigger_event"] else 0.0)
      + 1.00 * (1.0 if event_info["tokka_zone_entry"] else 0.0)
      + 1.20 * (1.0 if event_info["kakutei_engi"] else 0.0)
      + 0.30 * chain_continued_long
    ))


def update_emotions(persona, event_info, mt, initial_cash, stress_load, upset_recognition):
    """v4.2 感情モデル: 爆発項追加"""
    hit_signal = 1.0 if event_info["hit"] else 0.0
    ...
    # v4.2: 大逆転認知 × stress_load の爆発項（核心）
    explosion_term = 35.0 * upset_recognition * (0.5 + 1.5 * stress_load)
    raw_da += explosion_term

    raw_da *= persona.sensory_gating
    ...
    # v4.2: stress_load による絶望増幅 + 大逆転認知による救済
    raw_dd *= (1.0 + 0.6 * stress_load)
    raw_dd -= 8.0 * upset_recognition * stress_load
